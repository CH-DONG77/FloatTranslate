...
| **原文/译文** | 临时隐藏白块与译文，露出底层原文；再点一次恢复译文 |
| **⚙** | 设置：API Key、模型、目标语言、OCR 源语言、自动间隔 |
| **—** | 最小化为悬浮球（停靠桌面右下角，双击还原） |
| **×** | 关闭（自动记住窗口位置和大小） |

### 覆盖式翻译（本版本）

本版本采用**就地覆盖**翻译：识别到文字后，用白色方块盖住原文，把译文画在
原文位置上——没有单独的译文面板。

- **段落级上下文翻译**：把折行的多行 OCR 文本按段落合并后整段翻译（保留跨行
  上下文，不再逐行割裂），再把译文按各行宽度回流填回原位。
- **统一字号**：整段译文使用一致的字号，不会忽大忽小。
- 灰色识别区始终标记当前捕获范围；配合「自动」可持续跟随原文更新。
- **移动窗口后会自动清除白块和译文**（因为位置已与原文错开），重新点「翻译」即可。
- 点「**原文**」可临时移开覆盖层查看底层原文。
